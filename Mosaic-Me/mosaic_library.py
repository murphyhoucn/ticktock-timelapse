...
class MosaicGenerator:
    ...
    def __init__(self, input_dir, output_dir, target_width=16384, max_output_size=32768, white_border=0):
        """
        初始化马赛克生成器
        ...
        Args:
            ...
            white_border (int): 白边宽度（像素），默认0表示无边距
        """
        self.input_dir = Path(input_dir)
        self.output_dir = Path(output_dir)
        self.target_width = target_width
        self.max_output_size = max_output_size
        self.white_border = white_border
    ...
    def calculate_grid_layout(self, image_count, aspect_ratio=None):
        """
        计算最优网格布局
        
        Args:
            image_count (int): 图像数量
            aspect_ratio (float): 目标高宽比（行/列），例如16/9=1.78表示竖向16:9
        ...
        if aspect_ratio:
            # 根据指定的高宽比计算布局
            best_rows, best_cols = self._find_best_layout(image_count, aspect_ratio)
            rows, cols = best_rows, best_cols
            logger.info(f"目标高宽比: {aspect_ratio:.2f}:1, 实际: {rows/cols:.2f}:1")
        else:
            # 计算接近正方形的网格
        ...
        logger.info(f"网格布局: {rows}行 × {cols}列 = {rows * cols} 个单元格")
        logger.info(f"单元格大小: {cell_width}×{cell_height} 像素 (4:3比例)")
        if self.white_border > 0:
            total_width = self.white_border + (cell_width + self.white_border) * cols
            total_height = self.white_border + (cell_height + self.white_border) * rows
            logger.info(f"白边宽度: {self.white_border} 像素")
            logger.info(f"输出尺寸（含白边）: {total_width}×{total_height} 像素")
        else:
            logger.info(f"输出尺寸: {cols * cell_width}×{rows * cell_height} 像素")
        
        return rows, cols, cell_width, cell_height
    
    def _find_best_layout(self, image_count, target_ratio):
        """
        找到最接近目标比例且不超过图片数量的布局
        
        Args:
            image_count (int): 图像数量
            target_ratio (float): 目标高宽比（行/列）
        ...
        for cols in range(1, image_count + 1):
            ...
            if rows * cols > image_count:
                # 如果超出太多，跳过
                if rows * cols - image_count > cols:  # 超出一行以上
                    continue
    ...
    def create_mosaic_grid(self, image_files, rows, cols, cell_width, cell_height):
        ...
        # 创建空白画布（考虑白边）
        if self.white_border > 0:
            output_width = self.white_border + (cell_width + self.white_border) * cols
            output_height = self.white_border + (cell_height + self.white_border) * rows
        else:
        ...
        for idx, img_file in enumerate(image_files):
            ...
            try:
                ...
                with Image.open(img_file) as img:
                    ...
                    # 计算粘贴位置（考虑白边）
                    if self.white_border > 0:
                        x = self.white_border + col * (cell_width + self.white_border)
                        y = self.white_border + row * (cell_height + self.white_border)
                    else:
                        x = col * cell_width
                        y = row * cell_height
    ...
    def generate_mosaics(self):
        ...
        try:
            # 1. 生成网格马赛克
            logger.info("生成网格马赛克...")
            rows, cols, cell_width, cell_height = self.calculate_grid_layout(len(image_files), aspect_ratio=None)
