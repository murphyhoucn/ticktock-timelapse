...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Enhanced TickTock-Align-NPU Library')
    parser.add_argument('--input', '-i', default='NPU-Everyday-Sample', 
                       help='输入图像文件夹路径 (默认: NPU-Everyday-Sample)')
    parser.add_argument('--output', '-o', default='NPU-Everyday-Sample_EnhancedAlign', 
                       help='输出图像文件夹路径 (默认: NPU-Everyday-Sample_EnhancedAlign)')
    parser.add_argument('--reference', '-r', type=int, default=0, 
                       help='参考图像索引 (默认: 0，即第一张图像)')
    parser.add_argument('--night-threshold', '-t', type=int, default=80,
                       help='夜间图像判断阈值 (默认: 80)')
    ...
    print("   ╚═╝   ╚═╝ ╚═════╝╚═╝  ╚═╝   ╚═╝    ╚═════╝  ╚═════╝╚═╝  ╚═╝")
    print()
    print("Enhanced TickTock-Align-NPU Library")
    print("增强版建筑物图像序列对齐工具 - 专为日夜混合场景优化")
    print("=" * 60)
    
    # 创建增强版对齐器
    aligner = EnhancedTickTockAlign(
    ...
    # 设置夜间判断阈值
    aligner.night_threshold = args.night_threshold
